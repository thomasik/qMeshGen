...
#if !defined(SURFACEBSPLINEPLANAR_H__INCLUDED)
#define SURFACEBSPLINEPLANAR_H__INCLUDED
 ...
 */
class SurfaceBSplinePlanar : public SurfaceParametric  
{
public:
	/// Standard constructor
	SurfaceBSplinePlanar(const SurfacePlane& plane, const DRect& rect);
	/// Standard constructor
	SurfaceBSplinePlanar(const SurfacePlane& plane, DataMatrix<DPoint3d> *point_grid);
	/// Standard destructor
	virtual ~SurfaceBSplinePlanar();
public:
	virtual string getSimpleDescription() const override { return "bspline-planar"; }
...
public:
	...
	double fitToPoints(const DataVector<DPoint3d> & points, int pct, int init_res = 2);
	/// multi-fit to point-cloud using direct fit, using previous fit as a basis surface
	static std::shared_ptr<SurfaceMulti> multiFitToPoints(
		std::shared_ptr<SurfaceBSplinePlanar> base_surface, 
		const DataVector<DPoint3d> & points, 
		int min_level, int max_level, 
		double tolerance, double& max_dist);
	/// multi-fit to point-cloud using direct fit, using previous fit as a basis surface
	static std::shared_ptr<SurfaceMulti> multiFitToPointsRegular(
		std::shared_ptr<SurfaceBSplinePlanar> base_surface, 
...
#endif // !defined(SURFACEBSPLINEPLANAR_H__INCLUDED)
